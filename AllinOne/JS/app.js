...
setProductPictureLink = _id('set-product-picture-link') , setProductBuyLink = _id('set-product-buy-link') , setProductPrice = _id('set-product-price') , addProductButton = _id('add-product-btn') ,
...
async function addProductsToDOM (acces = 'Visitor') {
    ...
    if (acces == "Admin") {
        ...
        addProductArticle.addEventListener('click' , () => {addProductPage.style.display = 'flex' , addProductButton.textContent = 'Add' , clearAddProductForm()});
    ...
    await products.forEach(productProperties => {
        ...
        productPriceDollorSign.innerHTML = '$&nbsp;';
        ...
        buyProductButton.setAttribute('href' , productProperties.buyLink);
            ...
            editIcon.addEventListener('click' , async event => {
                ...
                await products.forEach(async (product , index) => {
                    if (product.title == event.target.parentElement.parentElement.firstElementChild.firstElementChild.textContent && product.description == event.target.parentElement.parentElement.firstElementChild.lastElementChild.textContent && product.pictureLink == event.target.parentElement.parentElement.previousElementSibling.firstElementChild.style.backgroundImage.substring(5 , event.target.parentElement.parentElement.previousElementSibling.firstElementChild.style.backgroundImage.length - 2) && product.buyLink == event.target.parentElement.previousElementSibling.lastElementChild.getAttribute('href') && product.price == Number(event.target.parentElement.previousElementSibling.firstElementChild.firstElementChild.textContent)) {
                        ...
                        setProductBuyLink.value = await product.buyLink;
                        ...
                        addProductButton.addEventListener('click' , async () => {
                            if (setProductTitle.value.length != 0 && setProductDescribe.value.length != 0 && setProductPictureLink.value.length != 0 && setProductBuyLink.value.length != 0 && setProductPrice.value.length != 0) {
                                await products.splice(index , 1 , {title: setProductTitle.value.toString().trim() , description: setProductDescribe.value.toString().trim() , pictureLink: setProductPictureLink.value.toString().trim() , buyLink: setProductBuyLink.value.toString().trim() , price: Number(setProductPrice.value)});
                                localStorage.setItem('Products' , JSON.stringify(products));
...
async function createProduct () {
    if (setProductTitle.value.length != 0 && setProductDescribe.value.length != 0 && setProductPictureLink.value.length != 0 && setProductBuyLink.value.length != 0 && checkUrl() && setProductPrice.value.length != 0) {
        ...
        (setProductBuyLink.value == 'https://' || setProductBuyLink.value == 'http://' ? setProductBuyLink.value = 'https://www.google.com' : null);
        await products.push({title: setProductTitle.value.toString().trim() , description: setProductDescribe.value.toString().trim() , pictureLink: setProductPictureLink.value.toString().trim() , buyLink: setProductBuyLink.value.toString().trim() , price: Number(setProductPrice.value)});
        localStorage.setItem('Products' , JSON.stringify(products));
        addProductPage.style.display = '';
...
function loginUser () {
    if (userName.value.toString().trim().length >= 4 && userPassword.value.length >= 4) {
        if (userName.value.toString().trim().toLowerCase() == "admin" && userPassword.value.toString() == "12345678") {
...
// ? Events
window.addEventListener('load' , () => (navigator.onLine ? _id('tab-icon').setAttribute('href' , 'Pic/Leaf - logo.png') : _id('tab-icon').setAttribute('href' , 'Pic/Leaf gray - logo.png')));
